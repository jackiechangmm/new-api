...
已登录用户可以使用内存中的 `Authorization: Bearer <Access Token>` 调用标准同步 `/v1` Relay。API token 调用继续使用原有认证和额度语义；Refresh Cookie 不直接用于 Relay 鉴权。
...
会话 Access Token 在进入 Relay 上游链路前会从请求头移除。标准同步 HTTP Relay、原生 Gemini `/v1beta/models/*` Relay，以及 canonical `/mj/submit/imagine`、`/mj/task/:id/fetch`、`/mj/task/list-by-condition` 支持会话调用。Midjourney 会话调用复用其现有钱包计费与失败退款语义，只保证不调整真实 API token 额度，不在此处获得标准同步 Relay 的订阅计费能力。`/v1/realtime`、模型发现、Suno、其他 Midjourney 动作、Midjourney 兼容路径和未实现兼容接口仍只接受 API token；`/v1/videos/:task_id/content` 的媒体下载另支持用户会话。
