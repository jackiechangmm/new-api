...
const casesSourceUrl =
  'https://api.github.com/repos/freestylefly/awesome-gpt-image-2/contents/data/cases.json'
const styleLibrarySourceUrl =
  'https://api.github.com/repos/freestylefly/awesome-gpt-image-2/contents/data/style-library.json'
const skillSourceUrl =
  'https://api.github.com/repos/freestylefly/awesome-gpt-image-2/contents/agents/skills/gpt-image-2-style-library/SKILL.md'
...
const promptsOutputPath = path.resolve('src/features/drawing/prompts-data.ts')
const styleLibraryOutputPath = path.resolve(
  'src/features/drawing/prompt-style-data.ts'
)
...
async function main() {
  const [casesResponse, styleLibraryResponse, skillResponse] =
    await Promise.all([
      fetchSource(casesSourceUrl),
      fetchSource(styleLibrarySourceUrl),
      fetchSource(skillSourceUrl),
    ])

  const [data, styleLibrary, skill] = await Promise.all([
    casesResponse.json(),
    styleLibraryResponse.json(),
    skillResponse.text(),
  ])
  if (
    !data ||
  ...
  if (
    !styleLibrary ||
    styleLibrary.repository !== expectedRepository ||
    !Array.isArray(styleLibrary.templates) ||
    !styleLibrary.templates.length ||
    !skill.trim()
  ) {
    throw new Error('提示词风格库格式无效')
  }

  const prompts = data.cases.map(normalizeCase)
  const promptsContent = `// 由 scripts/collect-drawing-prompts.mjs 生成，请勿手动编辑。\nexport const DRAWING_PROMPT_SOURCE = ${JSON.stringify(data.repository)} as const\nexport const DRAWING_PROMPT_CATEGORIES = ${JSON.stringify(data.categories, null, 2)} as const\nexport const DRAWING_PROMPT_STYLES = ${JSON.stringify(data.styles, null, 2)} as const\nexport const DRAWING_PROMPT_SCENES = ${JSON.stringify(data.scenes, null, 2)} as const\nexport const DRAWING_PROMPTS = ${JSON.stringify(prompts, null, 2)} as const\n`
  const styleLibraryContent = `// 由 scripts/collect-drawing-prompts.mjs 生成，请勿手动编辑。\nexport const DRAWING_PROMPT_SKILL = ${JSON.stringify(skill)} as const\nexport const DRAWING_PROMPT_TEMPLATES = ${JSON.stringify(styleLibrary.templates, null, 2)} as const\n`

  await Promise.all([
    mkdir(path.dirname(promptsOutputPath), { recursive: true }),
    mkdir(path.dirname(styleLibraryOutputPath), { recursive: true }),
  ])
  await Promise.all([
    writeFile(promptsOutputPath, promptsContent),
    writeFile(styleLibraryOutputPath, styleLibraryContent),
  ])
  const format = spawnSync(
    ...
    [
      ...
      '--write',
      promptsOutputPath,
      styleLibraryOutputPath,
    ],
    { stdio: 'inherit' }
  )
  if (format.status !== 0) throw new Error('提示词数据格式化失败')
  console.log(
    `已采集 ${prompts.length} 条图片提示词案例和 ${styleLibrary.templates.length} 个风格模板`
  )
}
