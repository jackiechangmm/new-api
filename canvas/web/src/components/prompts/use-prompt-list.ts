...
export function usePromptList({ keyword, tags, category, enabled = true }: { keyword: string; tags: string[]; category: string; enabled?: boolean }) {
    ...
    return {
        ...
        tags: useMemo(() => Array.from(new Set([ALL_PROMPTS_OPTION, ...(firstPage?.tags || [])])), [firstPage?.tags]),
        categories: useMemo(() => Array.from(new Set([ALL_PROMPTS_OPTION, ...(firstPage?.categories || [])])), [firstPage?.categories]),
